...
import { Button, Table, Dropdown, Input } from "antd";
...
const OnlineCourseManageContent = () => {
  ...
  const getClasses = async (getFirstPage) => {
    ...
    try {
      const { data } = await api.post(
        `/admin/class/get-data?page=${getFirstPage ? "1" : currentPage}`,
        {
      ...
      if (error.response.data.error === "There are no class in the system") {
        setData([]);
        setCurrentPage(1);
        setLastPage(1);
      }
  ...
  const items = [
    {
      label: (
        <a type="button" onClick={() => navigate(String(selectedClass))}>
          Edit
        </a>
      ),
      key: "0",
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "learners",
      render: (_, record) => {
        return <span>{JSON.parse(record.students).length}</span>;
      },
    },
    {
      title: "Class time",
