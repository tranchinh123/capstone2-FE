...
const EditAndCreateQuestion = () => {
  ...
  const handleDeleteAnswer = (_idx) => {
    const newQuestionInfo = {...questionInfo};
    newQuestionInfo.answers =  newQuestionInfo.answers.filter((_, idx) => idx !== _idx);
    setQuestionInfo(newQuestionInfo);
  }
  ...
  return (
    <>
     {openAIPanel && <AICreate open={openAIPanel} setOpen={setOpenAIPanel} handleAIQuestionCreate={handleAIQuestionCreate} />}
      {!id && (
              ...
              children: (
                ...
                >
                  <div>
                    ...
                    <ReactQuill theme="snow" modules={modules} value={question} onChange={handleChangeQuestion} />
                    {selectedQuestionType === 0 && (
                      <div className={styles.inputs}>
                        {questionInfo?.answers?.map((answer, idx) => (
                          <div className={styles.inputWrapper} key={idx}>
                            <span style={{ paddingTop: '5px' }}>{numberToLetter(idx + 1)}</span>
                            <Input value={answer} onChange={(e) => handleChangeAnswer(e, idx)} />
                            <FaRegTrashAlt className={styles.trashIcon} onClick={() => handleDeleteAnswer(idx)} />
                            <div className={styles.checkIcon} style={{ background: questionInfo?.correct_answer === idx ? 'green' : '#7d848b' }} onClick={() => handleCorrectAnswerChange(idx)}>
                  ...
                  <div>
                   ...
                   />
                    <Button type="primary" style={{ marginTop: '10px', width: '100%' }} onClick={handleCreateQuestion}>
                      {id ? 'Update' : 'Create'}
                    </Button>
                 ...
                 {selectedQuestionType === 0 && (
                   <Button type="text" style={{ fontWeight: 'bold' }} onClick={handleAddMoreAnswer}>
                    + Add choice
                   </Button>
                 )}
                </div>
