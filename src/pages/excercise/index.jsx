...
>>>>>>> doing-onl-off-excercise
  ...
  const handleDeleteExcercise = async () => {
    try {
      ...
      const { data } = await api.get(
        `/admin/excercise/destroy/${selectedExcerciseId}`
      );
      console.log(data, "data");
      window.showLoading(false);
      window.openNoti("Message", "Delete excercise successfully");
  ...
  const items = [
    {
      label: (
        <a type="button" onClick={() => handleGetInfoToEdit()}>
          Edit
        </a>
      ),
      key: "0",
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <span>
            {JSON.parse(record.excercise_content)
              .map((v) => +v.mark)
  ...
  const getExcercises = async (getFirstPage) => {
    try {
      ...
      const { data } = await api.post(
        `/admin/excercise/get-data?page=${getFirstPage ? "1" : currentPage}`,
        {
<<<<<<< HEAD
            label: (
                <a
...
>>>>>>> doing-onl-off-excercise
      ...
      if (
        error.response.data.error === "There are no excercises in the system"
      ) {
        setCurrentPage(1);
        setLastPage(1);
        setExcercises([]);
...
<<<<<<< HEAD
    ...
    return (
        <>
            {open && <CreateExcerciseModal isModalOpen={open} setIsModalOpen={setOpen} 
            listOfQuestions={listOfQuestions} getExcercises={getExcercises} selectedExcerciseInfo={selectedExcerciseInfo} />}
            <div
                style={{
                    display: 'flex',
...
=======
  const handleGetInfoToEdit = async () => {
    try {
      window.showLoading(true);
      const { data } = await api.get(
        `/admin/excercise/get-data/${selectedExcerciseId}`
      );
      setSelectedExcerciseInfo(data.excercise);
      setOpen(true);
      window.showLoading(false);
    ...
    <>
      {open && (
        <CreateExcerciseModal
          ...
          selectedExcerciseInfo={selectedExcerciseInfo}
        />
      )}
      <div
        style={{
