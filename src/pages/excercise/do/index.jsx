...
const DoingExcercisePage = ({ showFeedBack, hasComment = true, online }) => {
  const { api } = useAxios();
  ...
  console.log(searchParams.get('online'), 'asdasds');
  // const DUMMY = [
  //     {
  //         "question_id": 4,
  //         "mark": "4",
  ...
  const getExcercise = async () => {
    try {
      const { data } = await api.get(`/user/excercise-${searchParams.get('online') ? 'online' : 'offline'}/${courseId}/${excerciseId}`);
  ...
  const handleSubmitOnline = async (answerContent) => {
    try {
      const { data } = await api.post('/user/submit-excercise-online', {
        class_id: courseId,
  ...
  const handleSubmit = (time) => {
    if(searchParams.get('online')) {
      ...
      handleSubmitOnline(JSON.stringify(detail))
      return;
    }
  ...
  return (
    <>
     {questions && <DoingExcercise questions={questions} showFeedBack={showFeedBack} hasComment={hasComment} userAnswers={userAnswers} setUserAnswers={setUserAnswers} handleSubmit={handleSubmit} correctAnswers={correctAnswers} userMarks={userMarks} marks={marks} setUserMarks={setUserMarks} />}
...
export const DoingExcercise = ({ showFeedBack, hasComment, questions, userAnswers, setUserAnswers, handleSubmit, marks, userMarks, setUserMarks, correctAnswers, teacherComments, setTeacherComments }) => {
    ...
    <>
      ...
      {!showFeedBack && (
         <CountdownCircleTimer
            ...
            onUpdate={(remainingTime) => remainingTimeRef.current = remainingTime}
      ...
      <div style={{ maxHeight: '83vh', overflowY: 'scroll' }}>
       {questions.map((item, questionIndex) => (
         <Collapse key={questionIndex} items={[
          {
            key: '1',
            ...
            children: (
              <> 
               {item.question.question_type === 0 ? (
                <div style={{ display: 'grid', gridTemplateColumns: `${(showFeedBack && hasComment) ? '2fr 1fr' : '1fr'}`, gap: '20px' }}>
                  <div className="row">
                    <div className="col-xs-12">
                      <div className="mcq">
                        <h4 style={{ marginBottom: '10px' }}>
                          <span dangerouslySetInnerHTML={{ __html: item.question.question }} />
                        </h4>
                        {JSON.parse(item.question.answers).map((answer, answerIndex) => {
                          return (
                            <label key={answerIndex} className={`mcq-answer ${(showFeedBack && answerIndex === item.question.correct_answer) && 'correct'}`}>
                              {!showFeedBack && <input type="radio" name={questionIndex} id={answerIndex} onChange={handleMutipleChoice} />}
                              {showFeedBack && <input type="radio" name={questionIndex} id={answerIndex} checked={answerIndex === userAnswers[questionIndex]} />}
                              ...
                              {showFeedBack && (
                                <span className="feedback">
                                  <span lang="en">{(answerIndex === item.question.correct_answer) ? 'Correct answer' : 'Uncorrect answer'}</span>
                                </span>
                              )}
                            </label>
                          )
                        })}
                        {showFeedBack && <Alert message={item.question.explain} type="success" style={{ marginTop: '15px' }} />}
                      </div>
                    </div>
                  </div>
                  {(showFeedBack && hasComment) && (
                    <div style={{ border: '1px solid #DCDCDC', padding: '15px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                      <h5>Score:</h5>
                      <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                        <Input style={{ width: '60px' }} value={userMarks[questionIndex]} disabled /> 
                        <span>/</span>
                ...
                {(showFeedBack && hasComment) && (
                  <div style={{ border: '1px solid #DCDCDC', padding: '15px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    <h5>Score:</h5>
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                      <Input style={{ width: '60px' }} value={userMarks[questionIndex]} name={questionIndex} onChange={handleChangeScore} /> 
                      <span>/</span>
                      <span>{marks[questionIndex]} </span>
                    </div>
                    <h5>Comments:</h5>
