...
const CreateCourseForm = ({ propCourse }) => {
  ...
  const [finalExercise, setFinalExcercise] = useState(propCourse?.final_excercise);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setExercises(data.excercises.map(d => ({ value: d.id, label: d.excercise_name })))
      } catch (error) {
        console.log(error);
      }
    })()
  ...
  return (
    ...
    >
      ...
      >
        <Select
          disabled={propCourse ? true : false}
          // defaultValue="lucy"
